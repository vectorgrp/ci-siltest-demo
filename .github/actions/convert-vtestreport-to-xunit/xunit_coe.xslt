...
<!-- (c) 2014 Vector Informatik GmbH, Stuttgart                  -->
  ...
  <xsl:template match="*" mode="time">
    <xsl:variable name="timedelta" select="verdict/@endtimestamp - @timestamp"/>
    <xsl:value-of select="format-number($timedelta, '#.000')"/>
  </xsl:template>
  ...
  <xsl:template match="testcase[verdict[@result='fail' or @result='inconclusive' or @result='error']]" mode="verdict">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <!-- The text has to be all the way left. Unessecarry whitespaces are goint to cause parsing errors. -->
<xsl:text>
FAILED
</xsl:text>
       </failure>
